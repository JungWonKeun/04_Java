...
public class StudentView {
	...
	public void mainMenu() {
		...
		do {
			...
			case 4: selectName();  break; // 동명이인 X							
	...
	/**
	 * 이름을 입력 받아 service의 students 배열에서
	 ...
	 * ------------------------------------
	 * 이름 : 짱구
	 * HTML(80)점 CSS(70) JS(50) Java(90)
	 * 합계 : 290
	 * 평균 : 72.5
	 * ------------------------------------
	 */
	...
	private void testMaxMin() {		
		...
		for(int i = 0 ; i<arr.length ; i++) {
			...
			// 최대값 비교
			if(arr[i] > max) max = arr[i]; // max에 새 최대값을 대입
			
			// 최소값 비교
			if(arr[i] < min) min = arr[i]; // min에 새 최소값을 대입				
		}
