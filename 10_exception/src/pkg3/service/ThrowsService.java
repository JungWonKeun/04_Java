...
public class ThrowsService {
	 ...
	 * 해당 메서드를 호출하는 곳에서는 
	 * IOException에 대한 예외 처리 구문을 작성 해야만 한다(경고 문구같은 느낌)
	 * @param num
	 * @throws IOException
	 */
	...
	public void test5() {
		...
		do {
			...
			try {
				...
				input = -1;	// 첫 반복 시
				// 반복문이 종료되지 않도록 input값을 변경
				
			} catch(ArithmeticException e) {
				System.out.println("***** 발생한 산술적 예외를 test5()에서 처리 *****");
			
			} catch(Exception e) { // 다형성 (업캐스팅) 적용 가능
	...
	public void divNum() /* throws RuntimeException */ {
		...
		// num2가 0인 경우
		// ArthmeticException (산술적 예외) 발생 가능성이 있다!
		// -> 근데 왜 빨간줄이 안 뜨지??
		//    "UncheckedException" 이라서 빨간줄이 안 뜬다!!!
	 ...
	 *  - Scanner가 아닌 다른 방법 
	 *  
	 *  @throws IOException : 키보드 입력 시 발생할 수 있는 문제를 던진다 
	 */
