...
public class TryCatchService {
	...
	public void test2() {
		...
		try {
			...
			System.out.println("정수만 입력 하라니까???");
	...
	public void test3() {
		...
		try {
			...
			// -> InputMismatchException 만 처리한 경우
			//    0으로 나누었을 때 발생하는
	...
	public void test4() {
		...
		// -> 부모 타입을 나머지 예외를 처리하는(else) 형식으로 사용
			...
			// 장점 : 여러 예외 처리 구문을 작성하지 않고도
			// 		  예외 처리가 가능
			
			// 단점 : 발생하는 예외별 처리 방법을 제시할 수 없다			
			System.out.println("뭔진 모르겠지만 예외 발생");						
		} 
		...
		//    도달할 수 없는 코드(dead code/unreachable code)가 되어버림!
		
		// 해결 방법 : 부모 처리 구문 보다 앞에 작성
//		catch(InputMismatchException e ) {
//			System.out.println("int 범위 정수만 입력해 주세요");//			
//		}		
	 ...
	 *    - 예외 처리 구문 작성 여부를 필수가 아닌 선택하는 예외
